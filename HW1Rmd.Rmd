...
pop <- data.frame(prob = dhyper(0:2, 2, 6, 3), y = 0:2)

ggplot(data = pop, aes(y, prob)) + 
  geom_col()
```
Per the output, the probabilities for P(y = 0), P(y = 1), and P(y = 2) are 0.3571429, 0.5357143, and 0.1071429 respectively. 
...
sample_mean
```
The mean based on the probability distribution/population is 0.75, and the sample mean is 0.7. The sample mean does provide a good estimate of the population mean.
